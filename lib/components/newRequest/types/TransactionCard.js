...
import { AccountFunding } from 'uPortMobile/lib/components/Accounts/AccountFunding'
...
export class TransactionCard extends React.Component {
  ...
  renderFunding () {
    if (!(this.props.signerType !== "KeyPair" || (this.props.request.gasCost && (this.props.balance && this.props.balance > this.props.request.gasCost)) || (this.props.balance && this.props.request.value && this.props.balance > formatWeiAsEth(this.props.request.value)))) {
      return (<View style={{borderTopWidth: 1, borderBottomWidth: 1, borderTopColor: '#E0DFE6', borderBottomColor: '#E0DFE6', marginTop: 10, marginBottom: 10, padding: 10}}>
              <TouchableOpacity style={{paddingLeft: 32, flexDirection: 'row', justifyContent: 'space-between'}} onPress={() => this.toggleFunding()}>
                <View>
                  <Text style={{fontSize: 12}}>You will need to add funds to fully</Text>
                  <Text style={{fontSize: 12}}>interact with this app. Add funds now?</Text>
                </View>
                <EvilIcon size={30} name='chevron-right' color='rgba(0,0,0,1)' />
              </TouchableOpacity>
              </View>)
    }
  }
  ...
  pickScreen () {
    if (this.state.advanced) {
      return (
        <View style={{height: Dimensions.get('window').height, width: Dimensions.get('window').width, backgroundColor: '#FAFAFE'}}>
      ...
      return (
        <AccountFunding address={this.props.address} hexAddress={this.props.decodedMnid.address} network={networks[this.props.request.network] ? networks[this.props.request.network].name : this.props.request.network} close={this.toggleFunding} refreshBalance={this.props.refreshBalance} balance={this.props.balance} usdBalance={this.props.usdBalance} isModal={false}/>
      )
    } else {
      return (
        <View>
          <View style={{backgroundColor: '#F6F5FE', borderTopLeftRadius: 12, borderTopRightRadius: 12}}>
            ...
            <Text style={{fontFamily: 'Montserrat', color: colors.grey74, fontSize: 20, lineHeight: 26, marginLeft: 23}}>
            ...
            {this.props.request.abi
                ? <View style={{flexDirection: 'row', justifyContent: 'space-between', marginLeft: 8, marginRight: 8, backgroundColor: '#FFFFFF', paddingTop: 15, paddingBottom: 15, paddingLeft: 15, paddingRight: 15, shadowColor: 'rgb(200, 200, 200)', shadowOffset: { width: 0, height: 1.5 }, shadowOpacity: 0.5, shadowRadius: 16}}>
                  <Text style={{fontSize: Dimensions.get('window').width * 0.04}}>{this.props.request.abi.name.replace(/([A-Z])/g, ' $1').replace(/^./, function (str) { return str.toUpperCase() })}</Text>
                  <Text style={{fontSize: Dimensions.get('window').width * 0.04}}>{this.props.request.abi.args}</Text>
                </View> : null}
            {this.props.signerType === "KeyPair" && <View>
              <Text style={{marginLeft: 24, marginTop: 15, marginBottom: 5, fontSize: 10}}> NETWORK FEE </Text>
              <View style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginLeft: 8, marginRight: 8, backgroundColor: '#FFFFFF', paddingTop: 10, paddingBottom: 10, paddingLeft: 15, shadowColor: 'rgb(200, 200, 200)', shadowOffset: { width: 0, height: 1.5 }, shadowOpacity: 0.5, shadowRadius: 16}}>
                <Text style={{fontSize: Dimensions.get('window').width * 0.04}}>Average</Text>
                <View>
                  <Text style={{fontSize: Dimensions.get('window').width * 0.04}}> ${this.props.request.ethSpotPrice && this.props.request.gasCost ? (this.props.request.ethSpotPrice * this.props.request.gasCost).toFixed(2) : null } USD </Text>
                  <Text style={{fontSize: Dimensions.get('window').width * 0.03}}t> {this.props.request.gasCost ? this.props.request.gasCost : null} ETH </Text>
                </View>
              </View>
            </View>}
